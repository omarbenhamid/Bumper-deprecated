...
$result = $db->q("SELECT * FROM ".DBPREFIX."allowedEmails WHERE allowedEmail = ?", 's', $_POST['newEmail']);
if (is_array($result) and count($result) > 0) { echo 'That email address is already on the allowed list.'; die(); }
$result = $db->q("INSERT INTO ".DBPREFIX."allowedEmails (allowedEmail, timeZone, emailFormat) VALUES ( ?, ?, ? )", 'sss', $_POST['newEmail'], 'America/Anguilla', 'html');
...
$result = $db->q("DELETE FROM ".DBPREFIX."allowedEmails WHERE allowedEmail = ?", 's', $_POST['removeEmail']);
die();
endif;
...
$result = $db->q("UPDATE ".DBPREFIX."allowedEmails SET timeZone = ? WHERE allowedEmail = ?", 'ss', $_POST['timeZone'], $_POST['allowedEmail']);
...
$result = $db->q("UPDATE ".DBPREFIX."allowedEmails SET emailFormat = ? WHERE allowedEmail = ?", 'ss', $_POST['format'], $_POST['allowedEmail']);
